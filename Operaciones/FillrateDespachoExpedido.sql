...
FROM
    (SELECT W.WAVEKEY AS OLA,
     ...
     LEFT  JOIN WMWHSE51.PACK PK ON PK.PACKKEY = SK.PACKKEY AND SK.STORERKEY = PD.STORERKEY
     WHERE pd.STORERKEY like '%1770142%' and pd.status IN 
     (68,75,78,82,88,92,94,95,96,97)
     GROUP BY W.WAVEKEY,
              PD.ORDERKEY,
              SK.SKU,
              PD.SKU,
...
select DISTINCT status, '' from WMWHSE51.PICKDETAIL PD where STORERKEY like '%1770142%'